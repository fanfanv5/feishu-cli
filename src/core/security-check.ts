...
export function checkMultiAccountIsolation(cfg: ClawdbotConfig): IsolationStatus {
  ...
  const bindings = cfg.bindings as Array<{ match?: { channel?: string; accountId?: string } }> | undefined;
  const feishuBindings = bindings?.filter((b) => b.match?.channel === 'feishu' && b.match?.accountId);
  ...
  const agentIds = new Set(feishuBindings.map((b) => (b as any).agentId));
  if (agentIds.size === 1) {
    return {
